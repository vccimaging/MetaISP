...
def conv3x3(in_planes, out_planes, stride=1):
    ...
    return torch.nn.Sequential(
        ...
        nn.BatchNorm2d(out_planes)#BatchNorm2d
    )
...
class XCiT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768,
        ...
        self.fts_proj = nn.Conv2d(128,512,1,1,0)
        #self.head_2 = nn.Conv2d(128,512,1,1,0)
    ...
    def forward(self, x):
        ...
        x = self.fts_proj(torch.unsqueeze(torch.unsqueeze(z,2),2))
        return x

@register_model
def xcit_nano_12_p16(pretrained=False, pretrained_cfg=None,**kwargs):
    model = XCiT(
        patch_size=16, embed_dim=128, depth=4, num_heads=4, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), eta=1.0, tokens_norm=False, **kwargs)
    model.default_cfg = _cfg()
    return model

@register_model
def xcit_nano_12_p8(pretrained=False, pretrained_cfg=None, **kwargs):
    model = XCiT(
...
class PositionalEncodingFourier_(nn.Module):
    """
    Positional encoding relying on a fourier kernel matching the one used in the
    "Attention is all of Need" paper. The implementation builds on DeTR code
    https://github.com/facebookresearch/detr/blob/master/models/position_encoding.py

    Positional encoding slightly modified to allow for each patch have its own coordinates.
    Therefore, the coordinates are input and then projected by this module.
    """
    ...
    def forward(self, inp_coords):
        x_embed = inp_coords[:,0,:,:]
        y_embed = inp_coords[:,1,:,:]

        dim_t = torch.arange(self.hidden_dim, dtype=torch.float32, device=inp_coords.device)
...
class query_Attention(nn.Module):
    def __init__(self, dim, num_heads=2, qkv_bias=False, qk_scale=None, attn_drop=0.0, proj_drop=0.0):
        ...
        self.q = nn.Parameter(torch.rand((1, 1, dim)), requires_grad=True)
        self.q_proj = nn.Conv2d(dim, dim, 1, 1)
    ...
    def forward(self, x, q=None):
        ...
        if q is None:
            ...
            q = (self.q_proj(q).squeeze(2).permute(0,2,1)) * self.q.expand(B, -1, -1)
...
class CrossAdapter(nn.Module):
    def __init__(self, dim, num_heads, depth=1, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0.0,
                 attn_drop=0.0, drop_path=0.0, act_layer=nn.GELU, norm_layer=nn.LayerNorm,
                 att_strategy='adaquery', attention_match='prod',positional=False,adain=None):
        super().__init__()
        ...
        if positional:
            if dim < 128:
                ...
                h_dim = dim
            self.pos_embeder = PositionalEncodingFourier_(hidden_dim=h_dim//2,dim=dim)

        self.attention_strategy = att_strategy
        self.match_strategy = attention_match
        self.positional = positional

        self.attn = nn.ModuleList([query_Attention(dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale,
                    attn_drop=attn_drop, proj_drop=drop) for i in range(depth)])
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.norm2 = norm_layer(dim)
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer,
    ...
    def forward(self, x, style=None, coords=None):
        B, C, H, W = x.shape
        if self.positional:
            
            _,_,h,w = x.shape
            if x.shape[2] != coords.shape[2]:
                coords = F.interpolate(coords,([h,w]), mode='bilinear',align_corners=True)
            x = x + self.pos_embeder(coords)

        if self.adain != None:

            x = x.flatten(2).transpose(1, 2)

            if self.match_strategy=='sum':
                y = self.norm1(x)
        ...
        x = x + self.drop_path(self.norm3(self.mlp(self.norm2(x))))
