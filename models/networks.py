...
def get_scheduler(optimizer, opt):
	if opt.lr_policy == 'linear':
		...
		scheduler = lr_scheduler.CosineAnnealingLR(optimizer,
												   T_max=opt.niter,
												   eta_min=0)
	elif opt.lr_policy == 'tozero':
		lr_lambda = DecayLR(opt.niter, 0, opt.niter/2).step
...
def conv(in_channels=64, out_channels=64, kernel_size=3, stride=1, padding=1,
	...
	for t in mode:
		if t == 'C':
			...
			L.append(nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True))
		elif t == 'u':
...
class AttentionLayer(nn.Module):
	def __init__(self, out_channels, depth, heads, attention,attention_match='sum',positional=False,adain=None):
		super(AttentionLayer, self).__init__()

		self.attention = attention
		if attention == 'adaquery':
			self.channel_p = attention_models.CrossAdapter(dim=out_channels, num_heads=heads, depth=depth,  att_strategy=attention, attention_match=attention_match, positional=positional,adain=adain)
		else:
			assert 0, "Unsupported Attention: {}".format(attention)

	def forward(self, x, style=None, coords=None):
		if style is None:
			p = self.channel_p(x,coords=coords)

		else:
			p = self.channel_p(x,style,coords=coords)
...
class RCABlock(nn.Module):
	def __init__(self, in_channels=64, out_channels=64, kernel_size=3, stride=1,
				 padding=1, bias=True, mode='CRC', reduction=16, depth=4, heads=4, attention='channel',attention_match='sum',positional=False,adain=None):
		...
		if attention != 'channel' and attention != 'off':
			self.ca = AttentionLayer(out_channels, depth, heads, attention, attention_match, positional,adain)
	...
	def forward(self, x, style=None,coords=None):
		...
		if self.attention != 'off':
			if style is None:
				res = self.ca(res,coords=coords) 
			else:
				res = self.ca(res,style,coords=coords) 
		return res + x
...
class RCAGroup(nn.Module):
	def __init__(self, in_channels=64, out_channels=64, kernel_size=3, stride=1,
		...
		self.rg_0 = RCABlock(in_channels, out_channels, kernel_size, stride, padding,
				   bias, mode, reduction, depth, heads, attention,attention_match,positional,adain)

		self.conv = conv(out_channels, out_channels, mode='C')

	def forward(self, x, style=None,coords=None):
		if style is None:
			res = self.rg_0(x,coords=coords)
		else:
			res = self.rg_0(x,style,coords=coords)
