...
def augment_func(img, hflip, vflip):  # CxHxW
    if hflip:   img = img[:, :, ::-1]
    if vflip:   img = img[:, ::-1, :]
...
def augment(*imgs):  # CxHxW
    ...
    return (augment_func(img, hflip, vflip) for img in imgs)

def remove_black_level(img, black_lv=528, white_lv=4095):
...
def get_coord(H, W, x=448/2688, y=448/2688):
